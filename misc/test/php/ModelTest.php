...
class TestStore extends PersistentStore {
    ...
    public function __construct() {
        ...
        for ($i = 3; $i >= 1; $i--) {
            $pool = Pool::new();
            $hand = Hand::new(5);
            $this->player_infos[$i] = new PlayerInfo($i, 0, $hand, $pool, 0);
        }
...
END;
    ...
    public function testcheckPlayFirstPieceOfTurn_allowed(): void {
        ...
        $this->assertTrue($this->model->checkPlay(1, PieceType::MERCHANT, $this->ps->hex(2,0))->isAllowed());
        $this->assertTrue($this->model->checkPlay(1, PieceType::HIDDEN, $this->ps->hex(4,2))->isAllowed());
        $this->assertTrue($this->model->checkPlay(1, PieceType::FARMER, $this->ps->hex(4,2))->isAllowed());
        $this->assertTrue($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(6,2))->isAllowed());
    }
    ...
    public function testcheckPlayFirstPieceOfTurn_forbidden(): void {
        ...
        // not on a city
        $this->assertFalse($this->model->checkPlay(1, PieceType::MERCHANT, $this->ps->hex(1,1))->isAllowed());
        // not on a ziggurat
        $this->assertFalse($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(3,1))->isAllowed());
        // not on a friendly piece
        $this->assertFalse($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(4,0))->isAllowed());
        // not on an opponent piece
        $this->assertFalse($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(5,1))->isAllowed());
    }
    ...
    public function testPlayPiecesMoreThanTwoFarmers(): void {
        ...
        $this->assertTrue($this->model->checkPlay(1, PieceType::FARMER, $this->ps->hex(2, 4))->isAllowed());
        // but only a farmer
        $this->assertFalse($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(2, 4))->isAllowed());
        // also, can't play an extra farmer into the water
        $this->assertFalse($this->model->checkPlay(1, PieceType::FARMER, $this->ps->hex(4, 2))->isAllowed());
    }
    ...
    public function testPlayPiecesOnFields_noAdjacentPiece(): void {
        ...
        $this->assertFalse($this->model->checkPlay(1, PieceType::FARMER, $this->ps->hex(1,1))->isAllowed());
    }

    public function testPlayPiecesOnFields_nobleNoZigguratCard(): void {
        $this->setMap(ModelTest::MAP6);
        $this->assertFalse($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(1,1))->isAllowed());
    }
    ...
    public function testPlayPiecesOnFields_onlyAdjacentFarmer(): void {
        ...
        $m1 = $this->model->playPiece(0, RowCol::fromRowCol(2, 0));
        $this->assertTrue($this->model->checkPlay(1, PieceType::FARMER, $this->ps->hex(1,1))->isAllowed());
    }
    ...
    public function testPlayPiecesOnFields_loneNobleWithZigguratCard(): void {
        ...
        $result = $this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(1,1));
    ...
    public function testPlayPieces_threeNoblesOnLand(): void {
        ...
        $this->assertFalse($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(0, 2))->isAllowed());
        $this->assertFalse($this->model->checkPlay(1, PieceType::FARMER, $this->ps->hex(0, 2))->isAllowed());
        $this->model->playPiece(2, RowCol::fromRowCol(0, 2));
        $this->assertFalse($this->model->checkPlay(1, PieceType::PRIEST, $this->ps->hex(1,3))->isAllowed());
    }
    ...
    public function testPlayPieces_threeNoblesOneInWater(): void {
        ...
        $this->assertFalse($this->model->checkPlay(1, PieceType::PRIEST, $this->ps->hex(0, 2))->isAllowed());
        $this->assertFalse($this->model->checkPlay(1, PieceType::SERVANT, $this->ps->hex(0, 2))->isAllowed());
        $this->assertFalse($this->model->checkPlay(1, PieceType::FARMER, $this->ps->hex(0, 2))->isAllowed());
    }
